...
function friendlyError(
  err: unknown,
  action: "send" | "verify" | "options" | "password" | "forgot",
): string {
  ...
  if (action === "options")
    return "Could not check sign-in options. Try again.";
...
async function _loadSessionOnce(
  ...
  if (!profile) {
    ...
    console.warn(
      "[auth] Profile fetch failed with valid session — will auto-logout",
    );
    return {
      isLoggedIn: true,
      isLoading: false,
  ...
  if (profile.app_version !== APP_VERSION || profile.platform !== platform) {
    authApi
      .updateProfile({ app_version: APP_VERSION, platform })
      .catch(() => {});
  }
  ...
  const routeDistanceKm =
    ...
    fallbackRouteDistanceKm(
      profile.subscription_details?.tier ?? profile.subscription,
    );
  ...
  return {
    ...
    user: {
      ...
      subscription:
        profile.subscription_details?.tier ?? profile.subscription ?? "free",
      routeDistanceKm,
...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      console.warn(
        "[auth] Auto-logout triggered — profile could not be loaded after retries",
      );
      profileRetryCount.current = 0;
      if (refreshTimerRef.current) clearTimeout(refreshTimerRef.current);
  ...
  const updateUsername = useCallback(async (username: string) => {
    try {
      ...
      setState((s) => (s.user ? { ...s, user: { ...s.user, username } } : s));
      return true;
    } catch {
      return false;
    }
    ...
    try {
      ...
      setState((s) =>
        s.user
          ? {
              ...s,
              user: { ...s.user, disclaimer_accepted_at: result.accepted_at },
            }
          : s,
      );
      return true;
